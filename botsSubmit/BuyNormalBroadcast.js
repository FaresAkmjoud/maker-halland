            ...
            for (let file of readdirSync(folderPath2).filter(f => f.endsWith('.js'))) {
                ...
                client18.on("interactionCreate" , async(interaction) => {
                  ...
                  if(!normalBroadcast) {
                      ...
                      await tokens.add(`Broadcast2` , [{token:Bot_token,prefix:Bot_prefix,clientId:client18.user.id,owner:interaction.user.id,timeleft:2629744}])
                  }else {
                      await tokens.push(`Broadcast2` , {token:Bot_token,prefix:Bot_prefix,clientId:client17.user.id,owner:interaction.user.id,timeleft:2629744})
                  }
