  ...
  */
  async execute(interaction){
    if (interaction.isModalSubmit()) {
        if(interaction.customId == "BuyTicketbil_Modal") {
            ...
            try{
                ...
                let price1 = prices.get(`ticketbil_price_${interaction.guild.id}`) || 40;
...
const { Routes } = require('discord-api-types/v10');
               client00.on("ready" , async() => {
                ...
                .addFields(
                    ...
                    {
                        name:`**نوع البوت**`,value:`**\`تكت زر\`**`,inline:false
                    },
                    {
...
await usersdata.set(`balance_${interaction.user.id}_${interaction.guild.id}` , newbalance)
                const thebut = new ButtonBuilder().setLabel(`دعوة البوت`).setStyle(ButtonStyle.Link).setURL(`https://discord.com/api/oauth2/authorize?client_id=${client00.user.id}&permissions=8&scope=bot%20applications.commands`);const rowss = new ActionRowBuilder().addComponents(thebut);
                await interaction.user.send({embeds:[doneembeduser] , components:[rowss]})
            })
            
                let doneembedprove = new EmbedBuilder()
                .setColor(`Gold`)
                .setDescription(`**تم شراء بوت \`تكت زر\` بواسطة : ${interaction.user}**`)
                .setTimestamp()
                ...
                require("../../Bots/ticket/handlers/events")(client00)
                require("../../events/requireBots/ticketbil-commands")(client00);
                const folderPath = path.resolve(__dirname, '../../Bots/ticketbil/slashcommand00');
...
const folderPath3 = path.resolve(__dirname, '../../Bots/ticketbil/handlers');
for (let file of readdirSync(folderPath3).filter(f => f.endsWith('.js'))) {
    const event = require(path.join(folderPath3, file))(client00);
}
client00.on('ready' , async() => {
    setInterval(async() => {
      let BroadcastTokenss = tokens.get(`ticketbil`)
      let thiss = BroadcastTokenss.find(br => br.token == Bot_token)
      if(thiss) {
        if(thiss.timeleft <= 0) {
            console.log(`${client00.user.id} Ended`)
...
client00.on("ready" , async() => {
    ...
    const folderPath2 = path.resolve(__dirname, '../../Bots/ticketbil/events');
    ...
    client00.on("messageCreate" , async(message) => {
                  ...
                  await client00.login(Bot_token)
                  if(!ticketbil) {
                      await tokens.set(`ticketbil` , [{token:Bot_token,prefix:Bot_prefix,clientId:client00.user.id,owner:interaction.user.id,timeleft:2629744}])
                  }else {
                      await tokens.push(`ticketbil` , {token:Bot_token,prefix:Bot_prefix,clientId:client00.user.id,owner:interaction.user.id,timeleft:2629744})
                  }
