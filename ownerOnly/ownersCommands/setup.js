   ...
   const select = new StringSelectMenuBuilder()
    ...
    .addOptions(
        new StringSelectMenuOptionBuilder()
            ...
            new StringSelectMenuOptionBuilder()
            .setLabel(`حذف رابط`)
            .setEmoji(`<:20240604_113228:1247931819547033783>`)
...
await setting.set(`dashboard_room_${interaction.guild.id}` , dashboard.id)
 ...
 await setting.set(`statusroom_${interaction.guild.id}` , statusroom.id);
   ...
   if(!recipient && !line && !subscriberoom && !statusroom && !transferroom && !logroom  && !clientrole && !probot && !panelroom && !buybotroom && !dashboard && !proroom) return interaction.editReply({content:`**الرجاء تحديد اعداد واحد على الاقل**`}) 
